...
from STCILLM.conversation import conv_templates, SeparatorStyle
from STCILLM.utils import disable_torch_init
from transformers import CLIPVisionModel, CLIPImageProcessor, StoppingCriteria
from STCILLM.model import *
from STCILLM.model.utils import KeywordsStoppingCriteria
import json
import math
...
def eval_model(args, prompt_file, start_idx, end_idx):
    ...
    for idx, instruct_item in tqdm(enumerate(prompt_file)):
        ...
        with torch.inference_mode():
            ...
            ## hidden_states.shape [1,782,4096]
            ## start_ind
            if start_inx.shape[0] == 3:
                if hidden_states.shape[1] > start_inx[2] + feature_nums + 1:
            ...
            st_pre_load = st_pre_final[:, :, :, 0].squeeze().detach().cpu().tolist()
            st_pre_pv = st_pre_final[:, :, :, 1].squeeze().detach().cpu().tolist()
            st_pre_wind = st_pre_final[:, :, :, 2].squeeze().detach().cpu().tolist()



        x_load, y_load = st_data_x[:, :, region_start:region_end, 0].squeeze().tolist(), st_data_y[0, :, region_start:region_end,
                                                                           0].squeeze().tolist()
        x_pv, y_pv = st_data_x[:, :, region_start:region_end, 1].squeeze().tolist(), st_data_y[0, :, region_start:region_end,
                                                                             1].squeeze().tolist()
        x_wind, y_wind = st_data_x[:, :, region_start:region_end, 2].squeeze().tolist(), st_data_y[0, :, region_start:region_end,
                                                                             2].squeeze().tolist()        
        ...
        res_data.append(
            {"id": instruct_item["id"], "res": outputs, "x_load": x_load, "x_pv": x_pv, "x_wind": x_wind, "y_load": y_load, "y_pv": y_pv, "y_wind": y_wind,
             "st_pre_load": st_pre_load, "st_pre_pv": st_pre_pv, "st_pre_wind": st_pre_wind}.copy())
