...
class Block(nn.Module):
    def __init__(self, large_size, small_size, dmodel, dff, nvars, small_kernel_merged=False, drop=0.1):
        ...
        #convffn1
        self.ffn1pw1 = nn.Conv2d(in_channels=nvars * dmodel, out_channels=nvars * dff, kernel_size=1, stride=1,
                                 padding=0, dilation=1, groups=nvars)
        self.ffn1act = nn.GELU()
        self.ffn1pw2 = nn.Conv2d(in_channels=nvars * dff, out_channels=nvars * dmodel, kernel_size=1, stride=1,
                                 padding=0, dilation=1, groups=nvars)
        self.ffn1drop1 = nn.Dropout(drop)
        self.ffn1drop2 = nn.Dropout(drop)

        #convffn2
        self.ffn2pw1 = nn.Conv2d(in_channels=nvars * dmodel, out_channels=nvars * dff, kernel_size=1, stride=1,
                                 padding=0, dilation=1, groups=dmodel)
        self.ffn2act = nn.GELU()
        self.ffn2pw2 = nn.Conv2d(in_channels=nvars * dff, out_channels=nvars * dmodel, kernel_size=1, stride=1,
                                 padding=0, dilation=1, groups=dmodel)
...
class ModernTCN(nn.Module):
    def __init__(self, dims):
        ...
        # RevIN
        self.channelup_layers = nn.Conv2d(1, dims, kernel_size=(1, 1), stride=1)
        # backbone
        self.num_stage = 1
    ...
    def forward(self, x):
        for i in range(self.num_stage):
            ...
            x = x.reshape(B * C, N, L)
            x = x.unsqueeze(1)
            x = self.channelup_layers(x)
            _, D_, N_, L_ = x.shape
            x = x.reshape(B, C * D_, N_, L_)
        return x
...
class extendmodel(nn.Module):
    def __init__(self, configs):
        ...
        self.model = ModernTCN(dims=self.dims)
