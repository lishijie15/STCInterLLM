...
from STCILLM.model.st_layers import ST_Enc, parse_args
import json
...
class STLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if hasattr(config, "st_tower"):
            if self.config.st_tower == "Causal_Encoder":
                args = parse_args()
                # torch.backends.cudnn.enabled = False
                # torch.cuda.cudnn_enabled = True
                # torch.backends.cudnn.deterministic = False

                self.st_tower = ST_Enc(args)
    ...
    def forward(
        ...
        if st_tower is not None and (input_ids.shape[1] != 1 or self.training) and st_data_x is not None:
            if type(st_data_x) is list:
                ...
                pre_STE, STE_out = st_tower(st_data_x[0][..., :3])
                _, STE_lbls_out = st_tower(st_data_y[0][..., :3])
                if STE_out.shape[2] >= 1:
                    region_select_out = STE_out[:, :, region_start[0]:region_end[0], :].to(torch.bfloat16)
            else:
                pre_STE, STE_out = st_tower(st_data_x[..., :3])
                _, STE_lbls_out = st_tower(st_data_y[..., :3])
                region_select_out = STE_out[:, :, region_start[0]:region_end[0], :].to(torch.bfloat16)
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                ...
                if st_start_tokens.shape[0] >= 3:
                    ...
                    if cur_input_ids[
                        st_start_token_pos1 + num_patches] != st_tower.config.st_end_token:
